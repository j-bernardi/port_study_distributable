...
bounding_casing_suffix = 'ow' #suffix on the stl files form "NBPortn_(bounding_casing_suffix).stl"
...
def find_remainder(fused_object, port_name, port_number, port_obj, new_port_dict) :
	if port_name.startswith("NBPort" + port_number) :
		print port_name + ",",
		outer_pieces_shape = port_obj.cut(fused_object)
		print "outer pieces:", outer_pieces_shape
		inner_piece_shape = port_obj.common(fused_object)
		print "inner piece:", inner_piece_shape

		print "converting to solid"
		inner_piece = Part.makeSolid(inner_piece_shape)
		outer_pieces = Part.makeSolid(outer_pieces_shape)
		print "outer pieces:", outer_pieces_shape
		print "inner piece:", inner_piece_shape

		cutting_sphere = Part.makeSphere(magnitude(outer_pieces.CenterOfMass))

		outer_pieces = outer_pieces.cut(cutting_sphere)

		#fuse the remaining pieces..
		new_port_shape = inner_piece.fuse(outer_pieces)
		
		print "fragmented ports", fragmented_port
		print "old port:", port_dict[port]
		print "new port shape (fused fragmented):", new_port_shape
		print "making solid:"
		new_port = Part.makeSolid(new_port_shape)
		print "solid new port:", new_port
		
		new_port_dict[port_name] = new_port
...
#TODO : parallelise
def split_ports_remove_inner(port_dict, model_dict, first_wall_prefix, bounding_casing_suffix) :
	...
	for port in port_dict :
		if port[8:10] == bounding_casing_suffix :
			port_number = port[6:7]
			...
			for port in port_dict :
				if port.startswith("NBPort" + port_number) :
					...
					#print "pieces", pieces
					#print "og_box volume", port_dict[port].Volume
					#print "fused obj volume", fused_object.Volume
					#print "downgraded pieces", downgraded_pieces
					
					#print "Removing excess fragments..."
					
					for solid in downgraded_pieces :
						if solid.common(port_dict[port]).Volume == 0 :
							downgraded_pieces.remove(solid)

					#print "removed dg pieces", downgraded_pieces
					"""
					DIAGNOSTICS FOR REMOVING UNWANTED FRAGMENTS
					global current_directory_path_to_output
					i = 0
					for solid in downgraded_pieces :
						print solid, solid.Volume 
						print "exporting fragment to", current_directory_path_to_output + os.sep + 'fragments_' + str(i)
						solid.exportStl(current_directory_path_to_output + os.sep + 'fragments_' + port + '_' + str(i) + '.stl')
						i += 1
					"""
					...
					"""
					outer_pieces_shape = port_dict[port].cut(fused_object)

					print outer_pieces_shape 

					inner_piece_shape = port_dict[port].common(fused_object)
					...
					#TODO: Ditch the inner part..
					#find the 4th farthest vertex
...
def para_save(solid_dict, directory) :
	pass
...
for model_number in directories :
	...
	split_ports_remove_inner(port_dict, model_dict, first_wall_prefix, bounding_casing_suffix)
