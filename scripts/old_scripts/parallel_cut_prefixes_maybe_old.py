...
NOTE: should possibly do the cutting with the fragmented ports ie to avoid overlap with things like plasma, inside, when port doesn't exist
...
only_file_prefixes = ["BSS"]#["bss"] #empty string defaults to all
...
def split_ports_remove_inner(port_dict, model_dict, first_wall_prefix, bounding_casing_prefix, outer_casing_prefix) :
	...
	for port in port_dict :
		if port.lower().startswith(bounding_casing_prefix.lower()) :
			...
			for p in port_dict :
				if port_number in p :
					current_port_dict[p] = port_dict[p].copy()

			#print "current port dict", current_port_dict
			key_list_split = split_dict_to_key_parts(current_port_dict)
			#print "key list split:", key_list_split
			global count
			count = 0
			cpu_count = cpu_global_count
			q = Queue(maxsize = len(current_port_dict))
...
def fragment(key_list, port_dict, fused_object, port_number, outer_casing_prefix, q) :
	for port in key_list :
		if not port.startswith(outer_casing_prefix) : 
