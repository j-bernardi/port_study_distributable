"""
USAGE: 
Start with a few base-models (specifying radial angles) 
Then use the functions given to maniuplate lateral angles in a controlled way.
Calling print_models_to_file on a given list will append the models to the file found in model_files
"""
...
path_to_parameters = os.sep.join(script_dir_chop_up[:-1])+os.sep+"parameter_files/auto_parameters.txt"
...
#separation used for bunched ports
sep = 0.5
incs = 3
rands_per_model = 2
opposite_models = 4
min_lat, max_lat = -1.0, 1.0 #must be an intger on multiplcation by 100 for random number gen
