...
times = 8 #16 cap?
threads = 16 #ALWAYS LESS THAN 32
mem = "32g" #probably want 1g per core at least
#eg timesxthreads with mem
...
def output_head(model_number) :
    ...
    lines.append("#PBS -V")
    lines.append("#PBS -N serp_" + model_number)
    return lines

def processor_info(times, threads, mem, walltime) :
    lines = []
    #lines.append("#PBS -l nodes=" + str(nodes) + ":ppn=" + str(cores_per_node))
    lines.append("#PBS -l walltime=" + walltime)
    lines.append("#PBS -l select=" + str(times) + ":ncpus=" + str(threads) + ":mem=" + mem )
    return lines
    ...
    if not all_flag :
        if model in (starts_with_args) :
            for obj in parameter_dictionary[model] :
                ...
                output_lines += processor_info(times, threads, mem, walltime)
