...
#TODO: currently just selecting first density in list(ie points to first temperature) - in material cards - adjust
"""
USAGE:
Finds mosel descriptions in models/my_models/model_x
Can pass a range of directories as an argument, or will default to iterate through ALL directories
If specific enrichments and materials are required, pass flags -e (enrichment) -m (material)
Else will default to the defaults passed in auto_make_model_description.py

Outputs a serpent input file to the same directory as the model description (and runs if not commented out)
...
run = False #If true, makes input and runs. If false, makes input to be scp'ed and run on cum
...
#find the input filename based on arguments
#create a flag for specifying enrichment and blanket material, else if not present use default
#another argument for specifying model number

# dir change from specific to generic by shu
...
def return_serpent_file_material_cards(parts, script_dir):
...
def return_serpent_file_stl_parts(parts):
    ...
    lines_for_file.append('file '+component_name_of_source+'-b "'+source_CAD_file+'" 0.1 0 0 0  ' )
    lines_for_file.append('\n\n')
        ...
        try:
            ...
            for CAD_file, sub_component in zip(CAD_files_list, component_list):
                if sub_component == component and component != component_name_of_source:
                    lines_for_file.append('file '+component+'-b "'+CAD_file+'" 0.1 0 0 0  ' )
        except:
            pass
...
if __name__ == "__main__":
    ...
    for model in directories :
        ...
        serpent_file += return_serpent_file_stl_parts(parts)
        serpent_file += return_serpent_file_run_params()
        serpent_file += return_serpent_file_material_cards(parts, script_dir)
