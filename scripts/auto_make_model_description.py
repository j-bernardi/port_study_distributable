...
""" 
USAGE:
    Run with either 0 arguments (operate on all directories in my_models)
    or specify directories in argument
    eg:
        python auto_make_model_description.py 1
    makes a model description in models/my_models/model_1 for model_1/default_cut_model_1 (as default)
"""
#TODO: change so can add flags for enrichment arguments etc... If required
...
material_dict = {'bss': 'Eurofer' , 'coil': 'Homogenous_Magnet', \
                    ...
                    "ow" : 'SS-316L-IG', "div" : "Homogenised_Divertor"
                }
...
def set_parameters(parts, directory_path_to_stl_files) :
    """sets the plasma parameters from the dictionary found in Plasma_params.txt in the model directory"""
...
class Part(object):
    #Each model component is its own object
...
def output_file_info(parts, headers, filename, directory_path_to_output_dir):
    #outputs the info for each part in the parts dictionary to go under the headers in the same file
...
if __name__ == "__main__" :
    ...
    #Read in directories specified by args
    if len(sys.argv) > 1 :
    ...
    for model in directories :
        ...
        #define a relative path for running on cumulus etc for serpent inputs later
