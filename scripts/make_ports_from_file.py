...
""" 
USAGE: 
	Generates ports as STL fileswith parameters specified in parameter_file_name in parameter_directory.

	The script finds the tokamak parameters of the torus from the 'file finding parameters' section (from stl files in the model).
	...
	Saves these into a text file called previous_parameters.txt in the model's stl folder, if previous run has been done the script will find this and save this step.

	Outputs a port STL file directory to a directory (named by the model number read-in from parameter file) in my_models directory

Object description:
	The 'Port' object contains a list of component objects that describe a single port (ie outer wall, liner, etc..).
	The 'Component' object contains a 'box' member-state that is the freecad solid (amongst other descriptive data).
	'Parameter' object serves as a datastructure for reading in and storing data from the text file
	
	The bounding box for a given port is found with port.outer_casing_component.box.

See the bottom of the script for more possible outputs
"""

#########################################################################
###################### FILE FINDING PARAMETERS ##########################

toroid_parameter_prefix = 'BSS' #component from which Toroidal radii are found. (Ie to centre port on this radius)

parameter_directory = 'parameter_files' 
parameter_file_name = 'auto_parameters.txt' #default #'parameters.txt
...
def check_port_overlap(port_list) :
	...
	"""Checks whether any port in the port list overlaps with any other."""
...
def output_human_readable(path_to_my_models, model_number, output_content) :
	"""Output a human readable description of the model to its directory"""
...
def to_floats_and_check(lol) :
	"""Change strings in a list of lists (from the parameter file content) to floats, and check that all are the same length
	(eg same no of ports)"""
...
def make_new_directory(directory):
	"""maek a directory if one does not already exist"""
...
""" For each model in the parameter list, great the port geometry as specified in fucntions. Export to correct directory. """
